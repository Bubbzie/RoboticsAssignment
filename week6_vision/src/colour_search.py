...
class colour_search(object):
    ...
    def camera_callback(self, img_data):
        ...
        #            BLUE              GREEN
        lowers = [(115, 224, 100), (57, 240, 100)]
        uppers = [(130, 255, 255), (63, 260, 255)]

        #creating combined masks for all colours in lowers and uppers tuple list
        for i in range(2):
            if i == 0:
                mask = cv2.inRange(hsv_img, lowers[i], uppers[i])
            else:
                mask = mask + cv2.inRange(hsv_img, lowers[i], uppers[i])
